...
% pgfplots compatibility (remove the warning)
...
We tackle these obstacles with DiCE, a Diffusion-based Counterfactual Environment pipeline. The core insight is to treat modern text-guided diffusion in-painting models as causal interventions: by repainting only the background we synthesise counterfactual worlds that vary the environment $e$ while preserving the foreground object $f$ and label $y$. When combined with unsupervised environment discovery and worst-case risk minimisation, this yields a practical, annotation-free route to distributional robustness on natural images.
...
An image $x$ can be decomposed conceptually into a foreground object $f$, causally responsible for the label $y$, and a background environment $e$ that is spuriously correlated with $y$. In the training distribution $P_{\rho}(x,y)$ the correlation strength between $e$ and $y$ is $\rho$; at deployment this correlation may differ arbitrarily. We are given no environment labels and aim to learn parameters $\theta$ that maximise accuracy in the worst unseen environment.

\subsection{Notation}
Let $G = \{1,\ldots,K\}$ index latent environments discovered by clustering. For a mini-batch $B_g$ of images assigned to environment $g$ we write the average cross-entropy loss as
\[
  L_g(\theta) = \frac{1}{|B_g|} \sum_{(x,y)\in B_g} \ell\bigl(\theta; x, y\bigr).
\]
A Fourier regulariser $R_F(\theta)$ measures the fraction of gradient spectral energy above the 75th percentile.
...
The diffusion-generated images approximate interventional distributions $P\bigl(x\mid \mathrm{do}(e\leftarrow e')\bigr)$, providing the multiple environments required by IRM \cite{arjovsky-2019-invariant} without manual labels. Minimising the maximum empirical risk over these clusters coincides with discrete Wasserstein DRO \cite{gao-2016-distributionally}. The Fourier penalty discourages reliance on high-frequency artefacts linked to poor effective robustness \cite{fridovichkeil-2022-models}. Together they promote representations that ignore both background style and brittle spectral cues.

\section{Method}
\subsection{Pipeline overview}
1) \textbf{Foreground extraction:} For each training image, Segment-Anything (ViT-H) yields a binary mask $M$; pixels outside $M$ are marked for replacement. 2) \textbf{Diffusion counterfactuals:} Stable-Diffusion-2-Inpaint is conditioned on (a) the class name and (b) a randomly sampled text prompt from $\Pi = \{\text{``random landscape''}, \text{``urban street''}, \text{``mountain view''}, \text{``underwater scene''}\}$. Fifty sampling steps with guidance scale~7 produce four variants per prompt; variants whose CLIP similarity to the original foreground falls below~0.25 are discarded. 3) \textbf{Environment clustering:} For every real and accepted synthetic image we compute a CLIP-ViT/L-14 embedding after zeroing the foreground pixels; mini-batch K-means with $K = 8$ assigns an environment label $\hat g$. 4) \textbf{Robust objective:} In each optimisation step we solve
\[
  \min_{\theta}\; \max_{g\in G}\; w_g\, L_g(\theta) + \lambda\, R_F(\theta)
\]
subject to $w_g \ge 0$ and $\sum_{g} w_g = 1$. The inner maximisation updates $w_g$ via exponentiated gradient with step $\eta = 0.05$; the outer minimisation updates $\theta$ with AdamW. 5) \textbf{Online loop:} Every five epochs we estimate the causal effect $\hat{\tau} = |\mathrm{Cov}(\hat e, \hat y)|$ between one-hot cluster indicators $\hat e$ and model logits $\hat y$. If $\hat{\tau} > 0.05$ we generate additional counterfactuals for the top 20\,\% most biased samples and resume training.

\subsection{Training algorithm}
\begin{algorithm}[H]
  \caption{DiCE: Diffusion-based Counterfactual Environments with GC-DRO and Fourier penalty}
  \begin{algorithmic}[1]
    \State Initialise network parameters $\theta$, group weights $w_g \leftarrow 1/|G|$, replay buffer $\mathcal D$ with the training set
    \For{epoch $=1$ to $E$}
      \For{mini-batch $\mathcal B \subset \mathcal D$}
        \State Use SAM to obtain mask $M$ for each $(x,y) \in \mathcal B$
        \State With probability $p_{\text{gen}}$ sample prompt $p \sim \Pi$ and generate in-painted $\tilde x$ via Stable-Diffusion-2-Inpaint conditioned on the class and $p$
        \State \textbf{If} CLIP-foreground similarity $<0.25$ \textbf{then} discard $\tilde x$ \textbf{else} add $(\tilde x,y)$ to the batch
        \State For each image, zero out foreground pixels using $M$ and compute a CLIP embedding; assign cluster $\hat g$ by K-means
        \State Form per-group batches $\{\mathcal B_g\}$ and compute $L_g(\theta)$ for all $g$
        \State Compute Fourier regulariser $R_F(\theta)$ from gradient spectra
        \State Update group weights: $w_g \propto w_g\, \exp(\eta\, L_g(\theta))$, then normalise so that $\sum_g w_g = 1$
        \State Update $\theta$ with AdamW on objective $\sum_g w_g L_g(\theta) + \lambda R_F(\theta)$
      \EndFor
      \If{epoch mod $5 = 0$}
...
We monitor the dependence between predicted logits and discovered environments via $\hat{\tau} = |\mathrm{Cov}(\hat e, \hat y)|$. When $\hat{\tau}$ exceeds a threshold, we target the most biased samples for additional counterfactual generation, which re-balances clusters and tightens the worst-case objective.

\subsection{Hyper-parameters}
Unless stated otherwise: $K = 8$, $\lambda = 0.3$, $p_{\text{gen}} = 0.2$ synthetic sampling probability, batch size~256, learning rate~$3\times10^{-4}$, cosine schedule with 5-epoch warm-up. Values are tuned once at $\rho \approx 0.5$ and reused everywhere.
...
A) \textbf{Controlled correlation:} Waterbirds (\(\sim\)11k images) and CelebA-Hair, each resampled to six correlation levels $\rho \in \{0.10, 0.30, 0.50, 0.70, 0.85, 0.95\}$. Validation uses $\rho \approx 0.50$; test is the original balanced split. B) \textbf{Large-scale:} Train on ImageNet-1k, evaluate on ImageNet-V2, ImageNet-C (15$\times$5 severities), ImageNet-R, ImageNet-Sketch, DomainNet-Sketch and Counterfactual-CIFAR (ours). C) \textbf{Ablation:} CIFAR-10 and CIFAR-10-C severity~3.
...
Ninety epochs with cosine decay, 5-epoch warm-up; batch~256 (or~1024 for ViT); AdamW with $\beta=(0.9,0.999)$ and weight decay~0.05; gradient clipping at $\ell_2=1$. Hyper-parameters are grid-searched once and fixed. Five seeds (three for ImageNet) provide statistical confidence.

\subsection{Evaluation metrics}
Primary: worst-group accuracy (Acc$_{\mathrm{wg}}$) for suites~A and~C; mean corruption error (mCE) for ImageNet-C. Secondary: overall accuracy, robustness slope $\beta = |\partial \mathrm{Acc}_{\mathrm{wg}}/\partial \rho|$, high-frequency fraction (HFF), contrastive distortion (CD), effective robustness (ER), FGSM $\epsilon = 2/255$ accuracy, compute cost (FLOPs, GPU-hours). All metrics are logged per seed and reported as mean~$\pm$~sd.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.7\linewidth]{images/exp1_waterbirds_smoke_rho0.10_seed11.pdf}
  \caption{Validation accuracy during a three-epoch smoke test on Waterbirds with $\rho = 0.10$ (higher is better).}
  \label{fig:exp1_waterbirds_smoke_rho0.10_seed11}
\end{figure}
